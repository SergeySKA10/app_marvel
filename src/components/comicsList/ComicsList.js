...
const ComicsList = () => {
    const {comics, setOffsetComics, setPressBtnComics, newItemsLoading, dataEnded, onClearList, process} = useList();
    ...
    const styleBtn = dataEnded.comics ? {display: 'none'} : null;

    return (
        <div className="comics__list">

            { setList(process, () => createListComics(comics), newItemsLoading) }
            ...
            <div style={{display: 'flex'}}>
                <button className="button button__main button__long"
                        ...
                        onClick={() => {
                            ...
                            setPressBtnComics(true);
                        }}>
                    <div className="inner">load more</div>
                </button>
                <button className="button button__main button__long"
                        disabled={newItemsLoading}
                        onClick={() => onClearList('comics')}>
                    <div className="inner">
                            Clear list
                    </div>
                </button>
